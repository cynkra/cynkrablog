...
I then copied the results and passed them to `addMarkers()`. I faced somes challenges while trying to get the markers render well when zooming in and out. Be careful to fix the X an Y anchors and specify the size of the icon you use:
...
As a side note, the current GPX file indicates a duration of `r max(zch_gpx$routes[[1]]$Time) - min(zch_gpx$routes[[1]]$Time)` hours, which gives roughly 35km/h average speed, substantially lower than what the pro will do during the race.
...
There are 2 ways to rise the power. At low __velocity__ by putting more strength or increase the velocity while applying the same strength.
...
Therefore, a 58kg pro cyclist climber and 100kg pro track cyclist may have similar power ratio for a given duration, even though the former will be better for longer efforts. Talking about power without considering the effort __duration__ does not make much sense. World class women cyclist can sustain > 19W/kg during 5s (1360 for a 70kg athlete), men cyclist can sustain 24 W/kg during 5 seconds (2160W output for 90kg).
...
To proceed, we create a plot showing the __power__ as a function of the __distance__. We also
add the elevation profile in the background with `geom_area()` with a rather transparent alpha setting, so the user can focus on the power data. We add some `geom_smooth()` to see the relation between the power and distance ()`power ~ distance`) and display the mean power on an horizontal line with `geom_hline()`. On the second plot, we want to show the power distribution and leverage `geom_histogram`, the idea being to compare the mean power to the threshold power.
...
The ride was covered at `27.2km/h` average speed on an open road with wind and rain, definitely not the best conditions.
