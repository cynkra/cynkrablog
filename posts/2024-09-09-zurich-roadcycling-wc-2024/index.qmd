...
```{r, 3d-profil, eval=FALSE}
# 3D visualization code using rayshader (not executed in this rendering)
# Original code is preserved but not executed due to dependency issues
make_3d_plot <- function(dat, color_col) {
  ggplot(dat) +
    geom_path(aes(lon, lat, color = .data[[color_col]]), linewidth = 1) +
...
p <- make_3d_plot(df, "Elevation")
plot_gg(p, multicore = TRUE, raytrace = TRUE, width = 5, height = 4,
        scale = 250, windowsize = c(1400, 866), zoom = 0.4, phi = 30, theta = 30)
render_snapshot()
```

*Note: The 3D visualization using rayshader is not displayed in this rendering due to package dependency issues.*
...
```{r, rgl-route, eval=FALSE, warning=FALSE}
# 3D visualization code using ggrgl (not executed in this rendering)
# https://simplemaps.com/data/ch-cities
ch_cities <- readr::read_csv("ch.csv")
zh_cities <- ch_cities %>% filter(
  city %in% c("Zurich", "Winterthur", "Uster")
)
